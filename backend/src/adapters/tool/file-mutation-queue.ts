...
const FILE_MUTATION_LOCK_ROOT = join(tmpdir(), "nexus-file-mutation-locks");
...
async function readLockOwner(lockPath: string): Promise<LockOwner | null> {
  try {
    ...
    // Treat unreadable/malformed owner files as no owner.
  }
  return null;
}

async function removeDeadOrStaleLock(
  lockPath: string,
  nowMs: number,
): Promise<boolean> {
