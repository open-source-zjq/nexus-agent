/**
 * Cache prefix-volatility scanner.
 *
 * Detects tokens in a prompt prefix (system prompt + few-shot exemplars) that
 * are likely to change between otherwise-identical requests and therefore
 * defeat provider-side prompt caching. Volatile tokens include canonical UUIDs,
 * ISO-8601 timestamps (validated against the real calendar), hex hashes of
 * length 32/40/64, and JWTs (where the first two segments base64url-decode to
 * JSON objects).
 *
 * Pure functions, no dependencies beyond node builtins.
 */

import type { ImmutablePrefix, FewShotItem } from "./immutable-prefix.js";

export type VolatileKind = "uuid" | "iso8601" | "hex_hash" | "jwt";

/** Which part of the prefix a volatile token was found in. */
export type VolatileField = "systemPrompt" | "fewShots";

export interface VolatileFinding {
  field: VolatileField;
  kind: VolatileKind;
  token: string;
  /** Id of the few-shot item the token came from (only set for `fewShots`). */
  itemId?: string;
}

const HASH_LENGTHS: ReadonlySet<number> = new Set([32, 40, 64]);
const UUID_SEGMENT_LENGTHS: readonly number[] = [8, 4, 4, 4, 12];
const BOUNDARY_PUNCTUATION = ".,;:!?()[]{}<>\"'`";

/**
 * Scan an immutable prefix (system prompt + every few-shot item) for volatile
 * tokens. Returns every finding tagged with its field (and item id for
 * few-shots).
 */
export function detectVolatilePrefixContent(prefix: ImmutablePrefix): VolatileFinding[] {
  return [
...
function detectVolatileFewShotItem(item: FewShotItem): VolatileFinding[] {
  const text = fewShotText(item);
  if (!text) return [];
  return detectVolatileTokens("fewShots", text, item.id);
}

/** Project a few-shot item into the scannable text the original detector saw. */
function fewShotText(item: FewShotItem): string {
  switch (item.kind) {
    case "user_message":
    case "assistant_text":
...
function volatileTokenKind(token: string): VolatileKind | null {
...
function isJwt(token: string): boolean {
  ...
  return (
    isJsonObject(decodeBase64UrlJson(parts[0] ?? "")) &&
...
function isBase64UrlString(value: string): boolean {
  for (const char of value) {
    ...
    if (!numeric && !upper && !lower && char !== "-" && char !== "_" && char !== "=") {
      return false;
    }
  }
  return true;
