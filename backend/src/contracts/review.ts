...
export const ReviewTargetSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("uncommittedChanges") }),
  z.object({ kind: z.literal("baseBranch"), branch: z.string().trim().min(1) }),
  z.object({ kind: z.literal("commit"), sha: z.string().trim().min(1) }),
  z.object({ kind: z.literal("custom"), instructions: z.string().trim().min(1) }),
]);
