...
/**
 * Runtime capability manifest — the platform-level contract describing which
 * capabilities a serving runtime exposes, in what state, and with what limits.
 *
 * Ported faithfully from the original Nexus `contracts/capabilities`. The
 * intentional deviations are honored here: the Async/Mimo media gateway
 * protocol catalogs are NOT reintroduced — the media-generation capability
 * config is kept provider-neutral (generic baseUrl/apiKey/model) so the manifest
 * stays structurally faithful without re-adding the removed gateway.
 */

/** Bumped whenever the manifest shape changes in a breaking way. */
export const RUNTIME_CAPABILITY_CONTRACT_VERSION = 1;

/** Tri-state status for a capability: usable, switched off, or broken. */
...
/**
 * Base state machine attached to every capability entry in the manifest.
 * - `enabled`   : whether config turned the capability on.
 * - `available` : whether the capability is actually usable right now.
 * - `reason`    : human-readable explanation when disabled/unavailable.
 */
...
/**
 * Catalog of reasoning request protocols a model may speak. This is the
 * declaration enum only; the per-protocol `reasoning_effort` shaping logic is an
 * intentional deviation and is not implemented here.
 */
...
// --- Capability config (read by buildRuntimeCapabilityManifest) -------------
...
/** BM25 lexical search tuning for MCP tool discovery. */
...
/** A single MCP server entry with transport, trust scope, and validation. */
...
export type SubagentsCapabilityConfig = z.infer<typeof SubagentsCapabilityConfig>;
...
export const AttachmentsCapabilityConfig = CapabilityToggleConfig.extend({
  ...
  textFallbackMaxBase64Bytes: z
    .number()
    .int()
    .positive()
    .default(DEFAULT_ATTACHMENT_TEXT_FALLBACK_MAX_BASE64_BYTES),
  textFallbackMaxImageDimension: z
    .number()
    .int()
    .positive()
    .default(DEFAULT_ATTACHMENT_TEXT_FALLBACK_MAX_IMAGE_DIMENSION),
  textFallbackPreferredMimeType: z
    .string()
    .min(1)
    .default(DEFAULT_ATTACHMENT_TEXT_FALLBACK_PREFERRED_MIME_TYPE),
}).strict();
...
/**
 * Media-generation capability config. Kept provider-neutral (generic
 * baseUrl/apiKey/model) to honor the intentional deviation that drops the
 * Async/Mimo media gateway protocol catalogs.
 */
const MediaGenCapabilityConfig = CapabilityToggleConfig.extend({
  baseUrl: z.string().min(1).optional(),
  apiKey: z.string().min(1).optional(),
  model: z.string().min(1).optional(),
}).strict();

export const NexusCapabilitiesConfig = z
  .object({
    ...
    delegation: SubagentsCapabilityConfig.default(() => SubagentsCapabilityConfig.parse({})),
    ...
    imageGen: MediaGenCapabilityConfig.default(() => MediaGenCapabilityConfig.parse({})),
    speechGen: MediaGenCapabilityConfig.default(() => MediaGenCapabilityConfig.parse({})),
    musicGen: MediaGenCapabilityConfig.default(() => MediaGenCapabilityConfig.parse({})),
    videoGen: MediaGenCapabilityConfig.default(() => MediaGenCapabilityConfig.parse({})),
  })
  .strict();
export type NexusCapabilitiesConfig = z.infer<typeof NexusCapabilitiesConfig>;

/** Fully-defaulted capabilities config (everything OFF). */
export const DEFAULT_NEXUS_CAPABILITIES_CONFIG = NexusCapabilitiesConfig.parse({});
...
export const RuntimeCapabilityManifest = z
  .object({
    ...
    delegation: RuntimeCapabilityState.extend({
...
function providerCapabilityState(
  ...
  if (!enabled) {
    return { status: "disabled", enabled: false, available: false, reason: disabledReason };
  }
...
function webCapabilityState(
  ...
  if (!enabled) {
    return { status: "disabled", enabled: false, available: false, reason: "web access is disabled by config" };
  }
  if (fetchState.available || searchState.available) {
    return { status: "available", enabled: true, available: true };
  }
  return {
    status: "unavailable",
...
function skillsCapabilityState(
  ...
  if (!enabled) {
    return { status: "disabled", enabled: false, available: false, reason: "Skills are disabled by config" };
  }
  if (discoveredSkills > 0) {
    return { status: "available", enabled: true, available: true };
  }
  return {
    status: "unavailable",
...
function mcpCapabilityState(
  ...
  if (!enabled) {
    return { status: "disabled", enabled: false, available: false, reason: "MCP is disabled by config" };
  }
...
/** Runtime-observed signals layered on top of the static config. */
export interface RuntimeCapabilityManifestInput {
  config?: unknown;
  model: z.input<typeof ModelCapabilityMetadata>;
  mcp?: {
  ...
  delegation?: { available?: boolean; reason?: string };
  attachments?: { available?: boolean; reason?: string };
  memory?: { available?: boolean; reason?: string };
  imageGen?: { available?: boolean; reason?: string };
  speechGen?: { available?: boolean; reason?: string };
  musicGen?: { available?: boolean; reason?: string };
  videoGen?: { available?: boolean; reason?: string };
}

/**
 * Builds the runtime capability manifest by merging the static capabilities
 * config with runtime-observed signals (connected servers, discovered skills,
 * provider availability, ...).
 */
export function buildRuntimeCapabilityManifest(
  input: RuntimeCapabilityManifestInput,
): RuntimeCapabilityManifest {
  const config = NexusCapabilitiesConfig.parse(input.config ?? {});
  const configuredMcpServers =
    input.mcp?.configuredServers ?? Object.keys(config.mcp.servers).length;
  ...
  return RuntimeCapabilityManifest.parse({
    ...
    delegation: {
      ...providerCapabilityState(
        ...
        "delegation is disabled by config",
        input.delegation?.available === true,
        input.delegation?.reason ?? "delegation runtime is unavailable",
      ),
      maxParallel: config.delegation.maxParallel,
      maxChildRuns: config.delegation.maxChildRuns,
    },
    attachments: {
      ...providerCapabilityState(
