...
/** Truncate the assembled diff/prompt to this many UTF-8 bytes. */
export const DEFAULT_DIFF_MAX_BYTES = 256 * 1024;
const GIT_COMMAND_TIMEOUT_MS = 10_000;
const GIT_COMMAND_MAX_BUFFER = 384 * 1024;

export const REVIEW_WORKSPACE_NOT_GIT_MESSAGE =
  "Current workspace is not a Git repository. Code review requires a Git repository workspace.";

/**
 * Review target. Mirrors the original's discriminated union:
 * - workingTree:  staged + unstaged + untracked changes
 *                 (the canonical contract maps `uncommittedChanges` -> `workingTree`)
 * - baseBranch:   merge-base..HEAD against a branch
 * - commit:       a single commit
 * - custom:       free-form instructions (no git access)
 */
...
export interface ResolvedReviewTarget {
  title: string;
  /** The diff/prompt body fed to the model as the user prompt. */
  prompt: string;
}

export class ReviewWorkspaceNotGitError extends Error {
  readonly code = "workspace_not_git";
  readonly workspace: string;
  constructor(workspace: string) {
    super(REVIEW_WORKSPACE_NOT_GIT_MESSAGE);
    this.name = "ReviewWorkspaceNotGitError";
    this.workspace = workspace;
  }
}

export function isReviewWorkspaceNotGitError(error: unknown): error is ReviewWorkspaceNotGitError {
  return (
    error instanceof ReviewWorkspaceNotGitError ||
    Boolean(
      error &&
        typeof error === "object" &&
        (error as { code?: unknown }).code === "workspace_not_git",
    )
  );
}
...
export async function resolveReviewTargetPrompt(
  ...
  switch (options.target.kind) {
    case "workingTree":
      return resolveWorkingTree(workspace, maxDiffBytes);
    case "baseBranch":
...
async function assertGitWorkspace(workspace: string): Promise<void> {
  try {
    ...
    if (result.stdout.trim() !== "true") {
      throw new ReviewWorkspaceNotGitError(workspace);
    }
  } catch (error) {
    if (isReviewWorkspaceNotGitError(error)) throw error;
...
function isGitNotRepositoryError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  const lowered = message.toLowerCase();
  return (
    lowered.includes("not a git repository") ||
    lowered.includes("not a gitdir") ||
    lowered.includes("must be run in a work tree")
  );
}
